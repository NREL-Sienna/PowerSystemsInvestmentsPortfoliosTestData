...
df_hierarchy_nodal_add.rename(columns = {'st_x':'st'}, inplace = True)
...
df_hierarchy_nodal_add = df_hierarchy_nodal_add.reindex(columns=['Nodal', 'FIPS', 'BA', 'nercr', 'transreg', 'st', 'interconnect', 'country', 'usda', 'aggreg', 'county_name'])
...
df_hierarchy_nodal_add.rename(columns = {'Nodal':'*Nodal'}, inplace = True)
df_hierarchy_nodal_add
...
df_hierarchy_nodal_add.to_csv('C:\\Users\\akarmaka\\Documents\\GitHub\\ReEDS-2.0\\inputs\\hierarchy_rts_nodal.csv', sep=',', encoding='utf-8', index=False)
