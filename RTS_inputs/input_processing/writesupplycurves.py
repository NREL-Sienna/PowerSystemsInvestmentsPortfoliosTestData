...
def agg_supplycurve(scpath, inputs_case, numbins_tech, reeds_path, sw, val_r_all,
    ...
    if agglevel not in ['county','ba','nodal']:
...
def main(reeds_path,inputs_case,AggregateRegions=1,write=True,**kwargs):
    ...
    for s in ['ons','ofs']:
        ...
        if agglevel not in ['county','nodal']:
    ...
    if drscen != 'none':        
        ...
        dr_rsc = (tmp.groupby([c for c in tmp.columns if c not in ['value','year']])
                     .apply(grouper)
        )
        dr_rsc = (
    ...
    #spursites = spursites.loc[spursites.cnty_fips.isin(val_r_all)]
