...
if agglevel in ['ba','nodal']:
    print('all valid regions are BA or nodal, so skip aggregate_regions.py')
    quit()
else:
...
for filepath in inputfiles:
    ...
    elif (agglevel in ['state','ba'] and row['aggfunc']=='ignore') or (agglevel in ['county'] and row['disaggfunc']=='ignore'):
        if verbose > 1:
            logprint(filepath, 'ignored')
        continue    
