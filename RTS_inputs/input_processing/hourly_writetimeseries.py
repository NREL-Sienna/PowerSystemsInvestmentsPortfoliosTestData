...
def get_season_peaks_hourly(load, sw, reeds_path, inputs_case, hierarchy, h2szn, val_r_all):
    ...
    r_county = pd.read_csv(
        os.path.join(inputs_case,'r_county.csv'), index_col='county', squeeze=True)
    ...
    for r in load.r.unique():
        ...
        if agglevel == 'county':
            peak_szn = peakhour_agg_byseason[hierarchy.loc[r_county[r],sw['GSw_PRM_hierarchy_level']]]
        else:
...
def main(sw, reeds_path, inputs_case, periodtype='rep', make_plots=1, figpathtail=''):
    ...
    # inputs_case = os.path.join(reeds_path, 'runs', 'rts_run2_RTS', 'inputs_case')
    # sw = pd.read_csv(
