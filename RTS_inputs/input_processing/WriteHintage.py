...
def main(reeds_path, inputs_case):
    ...
    #      reeds_path,'runs','rts_run1_RTS','inputs_case')
    ...
    # value is present)
    agglevel = (
        pd.read_csv(os.path.join(inputs_case, 'agglevels.csv'),squeeze=True)
          .tolist())[0]
    ...
    val_r_all = pd.read_csv(
            os.path.join(inputs_case, 'val_r_all.csv'), header=None).squeeze().tolist()
...
#    indat = indat.loc[indat['reeds_ba'].isin(val_r_all)]
    indat['r'] = indat['reeds_nodal']
    ...
    dat = (df.groupby(['TECH', 'r', 'HR', 'resource.region', 'onlineyear', 
    ...
    elif agglevel == 'state': # or any other spatial resolution above 'BA'
    ...
    if make_plots:
        ...
        plt.savefig(os.path.join(inputs_case,
                                 'hintage_data_2020_BAs_with_max_num_units_of_each_tech.png'))
