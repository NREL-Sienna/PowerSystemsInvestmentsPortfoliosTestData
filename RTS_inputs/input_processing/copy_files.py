...
# inputs_case = os.path.join(reeds_path,'runs','rts_run9_RTS','inputs_case')
...
).rename(columns=lambda x: x.replace('*',''))#switched from county to nodal rename(columns={'*nodal':'nodal'})
#rename(columns={hierarchy.columns[0]:'*' + hierarchy.columns[0]}, inplace=True)
#switch to generic first column rename
if not NARIS:
...
agglevel = sw['GSw_Region_Resolution'].lower()
#agglevel should come out as nodal based on this switch update.
...
if '/' in sw['GSw_Region']:
    ...
    if len(GSw_Region) == 1:
        hier_sub = hierarchy[hierarchy[GSw_RegionLevel] == GSw_Region[0] ] 
    
    else: 
        for idx in range(0,len(GSw_Region)):
            if idx == 0:
                hier_sub = hierarchy[hierarchy[GSw_RegionLevel] == GSw_Region[idx]] 
            else:
                hier_sub = hier_sub.append(hierarchy[hierarchy[GSw_RegionLevel] == GSw_Region[idx]] )
...
#purpose of / here? multi region in same row?
...
#%%
    ...
    #Do not need to subset anything since nodal will be the lowest level of hierarchy
...
if (len(agglevels) > 1):
    ...
    lvl = 'ba' if agglevel in ['ba','state','county'] else 'nodal'
#change lvl to nodal

#%%
# Create an r column at the front of the dataframe and populate it with the
# county-level regions (overwritten if needed)
#For nodal is this requiured? Mapping will be.
hier_sub.insert(0,'r',hier_sub['nodal'])
...
hier_sub['nodal'].to_csv(
    os.path.join(inputs_case, 'val_nodal.csv'), header=False, index=False)
...
# Drop any substate region columns as these will no longer be needed - drop nodal as well since we have r now
hier_sub = hier_sub.drop(['nodal','county','ba','st_interconnect'],axis=1)
...
r_cs = r_cs[r_cs['*r'].isin(val_r_all)] #val_r_all check makes it work for nodal
#may need a better mapping file if this gets plotted
...
#nodal set has RE profiles / data - recheck with Wesley
if agglevel == 'county':
    # change from 'county' to nodal
    ...
    # Check to see if there is a file version file for existing county-leve
    ...
    for i,tech in enumerate(profile_data['tech']):
        # If the profile is already present, do nothing
        if profile_data['present'][i] == True:
            continue
        # Otherwise copy the profile over
        else:
            sc_path = profile_data['sc_path'][i]
            access_case = profile_data['access_case'][i]
            print('Copying county-level hourly profiles for {}'.format(tech))
            shutil.copy(
                os.path.join(
                    sc_path,'{}_county_{}'.format(tech,access_case),'results','{}.h5'.format(tech)),
                os.path.join(reeds_path,'inputs','variability','multi_year','{}-{}_county.h5'.format(tech,access_case))
            )
            # Update the file version information
            file_version_new.loc[file_version_new['tech'] == tech, 'file version'] = sc_path.split("/")[-1]
