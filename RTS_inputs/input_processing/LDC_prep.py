...
#         reeds_path,'runs','rts_run8_RTS','inputs_case')
    ...
    val_r_all = sorted(pd.read_csv(os.path.join(inputs_case, 'val_r_all.csv'), 
    ...
    # !!! TODO: CSP is hard-coded at ba resolution
    cspcf = read_file(
        os.path.join(path_variability, folder, 'csp-reference_{}'.format(lvl)))
    ...
    recf = pd.concat(
        [df_windons, df_upv, df_dupv] #df_windofs,
    ...
    if GSw_EFS1_AllYearLoad == 'historic':
        load_historical = read_file(os.path.join(basedir, 'inputs', 'loaddata',
                                                 'historic_load_hourly'))[val_r_all]
    ...
    #!!!!! Workaround until load data is updated to county-level
    #!!! TODO: It doesn't look like local_to_eastern is happening wiht load profiles.
    #load_eastern = local_to_eastern(load_profiles, reeds_path, by_year=True)
    load_eastern = load_profiles.copy()
    if agglevel == 'county':
        print('Disaggregating load data')
        # Reshape to long format
        load_cols = list(load_eastern.columns)
        load_eastern = (load_eastern.reset_index()
                                    .melt(id_vars=['year','hour'],value_vars=load_cols)
                                    .rename(columns={'variable':'r','value':'MW'})
                                    )
        # Disaggregate by population
        fracdata = pd.read_csv(os.path.join(inputs_case,'disagg_population.csv'), 
                               header=0)
        load_cols_long= list(load_eastern.columns)
        load_eastern = pd.merge(fracdata, load_eastern, left_on='PCA_REG', right_on='r', how='inner')
        load_eastern['new_value'] = (load_eastern['fracdata'].multiply(load_eastern['MW'], axis='index'))
        load_eastern.drop(columns=['MW']+['r'],inplace=True)
        load_eastern.rename(columns={'new_value':'MW','FIPS':'r'},inplace=True)
        load_eastern.set_index(load_cols_long[:-1],inplace=True)
        load_eastern = load_eastern[['MW']]
        # Put back in original format
        load_eastern = load_eastern.unstack('r')['MW']
