...
def identify_peak_containing_periods(df, hierarchy, r_county, level):
    """
    ...
    dfmod = df.copy()
    dfmod.columns = dfmod.columns.map(lambda x: x.split('_')[-1]).map(r_county).map(rmap)
...
def identify_min_periods(df, hierarchy, r_county, level, prefix=''):
    """
...
def main(sw, reeds_path, inputs_case, make_plots=1, figpathtail=''):
    ...
    r_county = pd.read_csv(
        os.path.join(inputs_case,'r_county.csv'), index_col='r', squeeze=True)
    ## Include r-to-r entries to speed up later merges
    r_county = pd.concat([r_county, pd.Series(index=r_county.unique(), data=r_county.unique())])
    #!!!! need to check why this is the case.. why do we not just load in r_county??
    r_county = r_county.drop_duplicates(keep='first')
    ...
    sc = {
        'wind-ons': pd.read_csv(
            ...
            f"wind-ons_supply_curve-{sw['GSw_SitingWindOns']}_{agglevel}.csv")
        ).groupby(['region','class'], as_index=False).capacity.sum(),
        'upv': pd.read_csv(
            os.path.join(reeds_path,'inputs','supplycurvedata',
            f"upv_supply_curve-{sw['GSw_SitingUPV']}_{agglevel}.csv")
        ).groupby(['region','class'], as_index=False).capacity.sum(),
    }
    sc = (
    ...
    if agglevel == 'county':
        sc['aggreg'] = sc.region.map(r_county).map(rmap)
    else:
    ...
    if sw['GSw_HourlyMinRElevel'].lower() not in ['false','none']:
        forceperiods_minre = {
            tech: identify_min_periods(
                df=recf, hierarchy=hierarchy, r_county=r_county,
    ...
    if agglevel == 'county':
        columns['region'] = columns.region.map(r_county).map(rmap)
    else:
    ...
    if sw['GSw_HourlyPeakLevel'].lower() not in ['false','none']:
        forceperiods_load = identify_peak_containing_periods(
            df=load, hierarchy=hierarchy, r_county=r_county, level=sw['GSw_HourlyPeakLevel'])
    else:
    ...
    if sw['GSw_PRM_StressSeedLoadLevel'].lower() not in ['false','none']:
        stressperiods_load = identify_peak_containing_periods(
            df=load, hierarchy=hierarchy, r_county=r_county, level=sw['GSw_PRM_StressSeedLoadLevel'])
    else:
    ...
    threads = pd.read_csv(
        os.path.join(inputs_case,'..','cplex.opt'), sep='\n', header=None, squeeze=True)
    threads = int(
        threads.loc[threads.str.startswith('threads')]
        .values[0].strip('threads= '))
    if threads <= 0:
        threads = 16
