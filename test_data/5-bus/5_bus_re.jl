...
ts_th_cheap_inv_capex = SingleTimeSeries("inv_capex", TimeArray(tstamp_inv, [1.0, coal_igcc_capex / coal_igcc_capex_2035]))
ts_th_exp_inv_capex = SingleTimeSeries("inv_capex", TimeArray(tstamp_inv, [1.0, coal_new_capex / coal_new_capex_2035]))
...
wind_op_costs = get_proportional_term.(get_value_curve.(get_variable.((get_operation_cost.((renewables))))))
...
for gen in renewables
    ...
    for (date, data) in ts.data
        if date == DateTime("2024-01-01T00:00:00")
            ts_wind_2030 += data * get_max_active_power(gen)
        else
            ts_wind_2035 += data * get_max_active_power(gen)
        end
    end
end
